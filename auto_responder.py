...
def generate_reply(
    ...
    attachments may contain plain URLs or dicts like:
    ...
    # Clean up any attachment types (dicts or strings) into URLs
    clean_urls: List[str] = []
    for att in attachments or []:
        if isinstance(att, str) and att.strip():
            clean_urls.append(att.strip())
        elif isinstance(att, dict) and "url" in att and att["url"].strip():
            clean_urls.append(att["url"].strip())
    ...
    for url in clean_urls:
        user_content.append({
            "type": "input_image",
            "image_url": url,   # ✅ must be a plain string, not an object
        })
