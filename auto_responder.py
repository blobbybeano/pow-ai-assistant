...
def _build_system_prompt(price_list: str, tone_profile: str) -> str:
    ...
    return (
        ...
        "- If the customer shares photos, include observations about the images and tailor your guidance accordingly.\n\n"
        "Tone guide:\n"
        f"{tone_profile}\n\n"
...
def generate_reply(
    ...
    limited_attachments = attachments[:4]
    if limited_attachments:
        summary_lines = []
        for index, attachment in enumerate(limited_attachments, start=1):
            summary_lines.append(
                f"Attachment {index}: {attachment.get('content_type', 'image')}"
            )
        user_content.append(
            {
                "type": "text",
                "text": "Customer provided the following attachments:\n"
                + "\n".join(summary_lines),
            }
        )
