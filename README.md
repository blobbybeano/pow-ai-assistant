...
No working credentials, customer records, training data, provider project files,
or business account identifiers are included. Copy `.env.example` to `.env` and
enter credentials for accounts you control. Configure pricing, tone, engineers,
calendar details, templates, and knowledge-base content through the app.

For Firebase, run `flutterfire configure` against your own project or supply the
Dart defines listed in `flutter_app/lib/firebase_options.dart`. See
[`SECURITY.md`](SECURITY.md) before deploying or publishing changes.
