...
class TwilioMessenger:
    ...
    def send_sms_message(self, *, to: str, body: str) -> str:
        """Send a plain SMS (no WhatsApp channel). No 24-hour window or template required."""
        if not body.strip():
            raise ValueError("Message body cannot be empty.")

        # Strip the whatsapp: prefix if present; SMS just needs a plain E.164 number
        to_number = to.replace("whatsapp:", "").strip()
        if not to_number.startswith("+"):
            raise ValueError(f"SMS recipient must be E.164 format, got: {to_number!r}")
        ...
        if self._config.messaging_service_sid:
            ...
            # Use the same number without the whatsapp: prefix
            from_plain = self._config.whatsapp_from.replace("whatsapp:", "").strip()
            kwargs["from_"] = from_plain
        else:
            raise RuntimeError("Twilio configuration missing messaging service SID or from number.")

        try:
            message = self._client.messages.create(**kwargs)
        except TwilioRestException as exc:
            raise RuntimeError(
                f"Failed to send SMS: {exc.msg} (status={exc.status}, code={exc.code})"
            ) from exc
...
def _normalize_whatsapp_address(value: Optional[str]) -> Optional[str]:
    """Normalize input like 07700 900123 to a WhatsApp E.164 address."""
    if not value:
        return None
