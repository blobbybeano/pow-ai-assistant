...
OPERATOR_CONTACTS_PATH = Path("operator_contacts.json")
PENDING_CONSULTATIONS_PATH = Path("pending_consultations.json")
...
def _load_consultations() -> dict:
    try:
        if PENDING_CONSULTATIONS_PATH.exists():
            return json.loads(PENDING_CONSULTATIONS_PATH.read_text(encoding="utf-8"))
    except Exception:
        pass
    return {}
...
def _save_consultations(data: dict) -> None:
    PENDING_CONSULTATIONS_PATH.write_text(json.dumps(data, indent=2, default=str), encoding="utf-8")


def _is_operator_number(phone: str) -> bool:
    """Return True if this phone belongs to an active operator."""
    normalized = _normalize_msisdn(phone)
    return any(
        op.get("active") and _normalize_msisdn(op.get("phone", "")) == normalized
...
def _handle_operator_reply(
    ...
    """
    If this message is an operator reply to a pending consultation, route
    the answer back to the customer and return True (so normal flow is skipped).
    """
    normalized_from = _normalize_msisdn(from_number)
    consultations = _load_consultations()

    # Find pending consultations assigned to this operator
    pending = [
        (cid, c) for cid, c in consultations.items()
        if _normalize_msisdn(c.get("operator_number", "")) == normalized_from
        and c.get("status") == "pending"
    ]
    if not pending:
        return False

    # Take the most recent pending consultation
    pending.sort(key=lambda x: x[1].get("timestamp", ""), reverse=True)
    consult_id, consultation = pending[0]

    # Mark answered
    consultations[consult_id]["status"] = "answered"
    consultations[consult_id]["answer"] = inbound_text
    consultations[consult_id]["answered_at"] = str(_iso_now())
    _save_consultations(consultations)

    customer_number = consultation.get("customer_number", "")
    original_message = consultation.get("original_message", "")
    question = consultation.get("question", "")
    cust_account_id = consultation.get("account_id") or account_id

    # Build a context-aware reply for the customer
    context_msg = (
        f"[SYSTEM NOTE — do not mention this note to the customer] "
        f"An operator has answered a question you consulted them about.\n"
        f"Your question was: {question}\n"
        f"Operator's answer: {inbound_text}\n\n"
        f"Now use this answer to continue helping the customer. "
        f"The customer's original message was: {original_message}\n"
        f"Reply naturally and helpfully — do not reveal that you consulted an operator."
    )

    try:
        customer_reply = _build_reply(context_msg, None, account_id=cust_account_id)
        # Strip any accidental CONSULT_OPERATOR tokens
        customer_reply = re.sub(r"\[CONSULT_OPERATOR:.*?\]", "", customer_reply, flags=re.DOTALL).strip()
    except Exception:
        logging.exception("Failed to build reply from operator answer")
        customer_reply = (
            "Thanks for your patience! I've just received some information from our team. "
            "Would you like me to continue with a quote based on the details you've shared? 😊"
        )

    messenger = get_twilio_messenger(cust_account_id)
    if messenger and customer_number:
        try:
            messenger.send_whatsapp_message(to=f"whatsapp:{customer_number}", body=customer_reply)
            conversation_store.record_message(
                cust_account_id, customer_number,
                text=customer_reply, author="ai", direction="outbound",
            )
            logging.info("Operator answer routed to customer %s (consultation %s)", customer_number, consult_id)
        except Exception:
            logging.exception("Failed to send operator-answered reply to customer %s", customer_number)

    return True


def _send_operator_consultation(
    *,
    question: str,
    customer_number: str,
    customer_name: str,
    ...
    """Send a consultation WhatsApp to all active operators and track in pending_consultations.json."""
    import json as _json
    from uuid import uuid4 as _uuid4

    ops = [op for op in _load_operators() if op.get("active")]
    if not ops:
        return

    consult_id = f"consult_{_uuid4().hex[:8]}"
    consultations = _load_consultations()

    for op in ops:
        op_phone = _normalize_msisdn(op.get("phone", ""))
        if not op_phone:
            continue

        wa_msg = (
            f"🔔 *PowWash — Customer Query*\n\n"
            f"*Customer:* {customer_name or customer_number}\n"
            f"*Question:* {question}\n\n"
            f"Reply to this message with your answer — it will go straight back to the customer automatically.\n"
            f"_(Ref: {consult_id})_"
        )
        # Plain-text version for SMS fallback (no WhatsApp markdown)
        sms_msg = (
            f"PowWash - Customer Query\n\n"
            f"Customer: {customer_name or customer_number}\n"
            f"Question: {question}\n\n"
            f"Reply with your answer and it goes straight to the customer. (Ref: {consult_id})"
        )
        sent_via = None
        try:
            messenger.send_whatsapp_message(to=f"whatsapp:{op_phone}", body=wa_msg)
            sent_via = "WhatsApp"
        except Exception as wa_exc:
            # Twilio error 63016 = outside 24-hour free-form window.
            # Any WhatsApp failure falls back to SMS — no templates, no extra cost.
            logging.warning(
                "WhatsApp to operator %s failed (%s) — falling back to SMS",
                op_phone, wa_exc,
            )
            try:
                messenger.send_sms_message(to=op_phone, body=sms_msg)
                sent_via = "SMS"
            except Exception:
                logging.exception("SMS fallback also failed for operator %s", op_phone)

        if sent_via:
            logging.info(
                "Consultation %s sent to operator %s (%s) via %s",
                consult_id, op.get("name"), op_phone, sent_via,
            )

        consultations[consult_id] = {
            "customer_number":   customer_number,
            "customer_name":     customer_name,
            "operator_number":   op_phone,
...
def whatsapp_webhook() -> Response:
    ...
    # ── Operator reply fast-path ──────────────────────────────────────────
    # If the message is from a known active operator, route it as an answer
    # to a pending customer consultation and skip the normal AI flow.
    if _is_operator_number(from_number):
        if _handle_operator_reply(from_number, inbound_text, account_id):
            return Response(str(MessagingResponse()), mimetype="application/xml")
        # Operator message but no pending consultation — fall through to normal flow
    # ─────────────────────────────────────────────────────────────────────
    ...
    if ai_enabled:
        ...
        # ── CONSULT_OPERATOR signal ───────────────────────────────────────────
        consult_match = re.search(r"\[CONSULT_OPERATOR:\s*(.*?)\]", reply_text, re.DOTALL)
        if consult_match:
            question = consult_match.group(1).strip()
            messenger = get_twilio_messenger(account_id)
            if messenger:
                _send_operator_consultation(
                    question=question,
                    customer_number=conversation_id,
                    customer_name=profile_name or conversation_id,
                    original_message=inbound_text,
                    account_id=account_id,
                    messenger=messenger,
                )
            # Replace the signal tag with just the customer-facing holding message
            reply_text = re.sub(r"\[CONSULT_OPERATOR:.*?\]", "", reply_text, flags=re.DOTALL).strip()
            if not reply_text:
                reply_text = "I'm just checking a couple of details with our team — I'll come back to you shortly! 😊"
        # ─────────────────────────────────────────────────────────────────────
