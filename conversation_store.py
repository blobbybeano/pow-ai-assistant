...
class ConversationStore:
    ...
    def delete_conversation(self, account_id: Optional[str], conversation_id: str) -> None:
        acct = self._account_id(account_id)
        convo_ref = self._conversation_ref(acct, conversation_id)
        # Delete messages subcollection in batches
        msgs_ref = self._messages_query(acct, conversation_id)
    ...
    def update_message_by_transport_sid(
        self, account_id: Optional[str], transport_sid: str, *, status: Optional[str] = None
    ) -> Optional[MessageRecord]:
        """Find a message by its Twilio MessageSid and update its status."""
        acct = account_id or self._default_account_id or ""
        query = self._client.collection_group("messages").where("transportSid", "==", transport_sid)
        if acct:
            query = query.where("accountId", "==", acct)
        for doc in query.stream():
            updates: Dict[str, Any] = {}
            ...
            return self._message_from_snapshot(doc.reference.get())
        return None


class _InMemoryConversationStore:
    """
    In-memory fallback used when Firebase/Firestore is not configured.
    Conversations are lost on server restart — fine for local testing.
    """

    def __init__(self, *, default_account_id: Optional[str] = None) -> None:
        import threading
        self._lock = threading.Lock()
        self._default_account_id = (default_account_id or "demo").strip() or "demo"
        self._conversations: Dict[str, Dict[str, ConversationRecord]] = {}
        self._messages: Dict[str, Dict[str, List[MessageRecord]]] = {}
        self._default_responders: Dict[str, Optional[str]] = {}

    @property
    def default_account_id(self) -> Optional[str]:
        return self._default_account_id

    def _acct(self, account_id: Optional[str]) -> str:
        return (account_id or self._default_account_id or "demo").strip() or "demo"

    def list_conversations(self, account_id: Optional[str] = None) -> List[Dict[str, Any]]:
        acct = self._acct(account_id)
        with self._lock:
            convos = list(self._conversations.get(acct, {}).values())
        result = [c.to_summary() for c in convos]
        result.sort(
            key=lambda c: (c.get("lastMessage") or {}).get("timestamp") or "",
            reverse=True,
        )
        return result
    ...
    def get_conversation(
        ...
        acct = self._acct(account_id)
        with self._lock:
            convo = self._conversations.get(acct, {}).get(conversation_id)
            if not convo:
                return None
            if mark_read:
                convo.unread_count = 0
            messages = list(self._messages.get(acct, {}).get(conversation_id, []))
        return convo.to_dict(messages)
    ...
    def get_default_responder(self, account_id: Optional[str] = None) -> Optional[str]:
        acct = account_id or self._default_account_id
        return self._default_responders.get(acct or "")

    def set_default_responder(self, responder_id: Optional[str], *, account_id: Optional[str] = None) -> None:
        acct = self._acct(account_id)
        self._default_responders[acct] = (responder_id or "").strip() or None
    ...
    def record_message(
        ...
        *,
        ...
        record = MessageRecord(
            id=message_id, text=text, author=author, direction=direction,
            timestamp=now, via=via, transport_sid=transport_sid, status=status,
            scheduled_send_at=scheduled_send_at, sent_at=sent_at, error=error,
            attachments=list(attachments or []),
            account_id=acct, conversation_id=conversation_id,
        )
        with self._lock:
            if acct not in self._conversations:
                self._conversations[acct] = {}
            if acct not in self._messages:
                self._messages[acct] = {}
            if conversation_id not in self._conversations[acct]:
                self._conversations[acct][conversation_id] = ConversationRecord(
                    id=conversation_id,
                    phone_number=conversation_id,
                    contact_name=profile_name,
                    contact_photo_url=profile_photo_url,
                    ai_enabled=True,
                    unread_count=0,
                    account_id=acct,
                )
            convo = self._conversations[acct][conversation_id]
            if profile_name and not convo.contact_name:
                convo.contact_name = profile_name
            if increment_unread:
                convo.unread_count += 1
            convo.last_message = record
            if conversation_id not in self._messages[acct]:
                self._messages[acct][conversation_id] = []
            self._messages[acct][conversation_id].append(record)
        return record

    def latest_customer_message(self, account_id: Optional[str], conversation_id: str) -> Optional[MessageRecord]:
        acct = self._acct(account_id)
        with self._lock:
            msgs = self._messages.get(acct, {}).get(conversation_id, [])
            for m in reversed(msgs):
                if m.author == "customer":
                    return m
        return None

    def pending_scheduled_messages(
        self, *, account_id: Optional[str] = None
    ) -> Iterable[Tuple[str, str, MessageRecord]]:
        acct = account_id or self._default_account_id
        with self._lock:
            snapshot = {
                aid: {cid: list(msgs) for cid, msgs in convos.items()}
                for aid, convos in self._messages.items()
            }
        for aid, convos in snapshot.items():
            if acct and aid != acct:
                continue
            for cid, msgs in convos.items():
                for m in msgs:
                    if m.author == "ai" and m.status in ("scheduled", "drafting"):
                        yield aid, cid, m
    ...
    def get_message(
        ...
        acct = self._acct(account_id)
        with self._lock:
            for m in self._messages.get(acct, {}).get(conversation_id, []):
                if m.id == message_id:
                    return m
        return None
            ...
            for m in self._messages.get(acct, {}).get(conversation_id, []):
                if m.id == message_id:
                    ...
                    if sent_at is not None:
                        m.sent_at = sent_at
                    if transport_sid is not None:
                        m.transport_sid = transport_sid
                    if error is not None:
                        m.error = error
                    if scheduled_send_at is not None:
                        m.scheduled_send_at = scheduled_send_at
                    if attachments is not None:
    ...
    def cancel_scheduled_message(
        ...
        return self.update_message(
            ...
            status="cancelled", scheduled_send_at=None, error="Cancelled by agent",
        )
    ...
    def delete_conversation(self, account_id: Optional[str], conversation_id: str) -> None:
        acct = self._acct(account_id)
        with self._lock:
            self._conversations.get(acct, {}).pop(conversation_id, None)
            self._messages.get(acct, {}).pop(conversation_id, None)

    def update_message_by_transport_sid(
        self, account_id: Optional[str], transport_sid: str, *, status: Optional[str] = None
    ) -> Optional[MessageRecord]:
        """Find a message by Twilio SID across all conversations and update its status."""
        acct_filter = (account_id or self._default_account_id or "").strip()
        with self._lock:
            accounts = [acct_filter] if acct_filter else list(self._messages.keys())
            for aid in accounts:
                for msgs in self._messages.get(aid, {}).values():
                    for m in msgs:
                        if m.transport_sid == transport_sid:
                            if status is not None:
                                m.status = status
                            return m
        return None
...
try:
    conversation_store: ConversationStore | _InMemoryConversationStore = ConversationStore()
except Exception:
    logger.warning(
        "Firebase/Firestore not available — conversations stored in memory only (demo mode). "
        "Configure FIREBASE_CREDENTIALS_FILE to enable persistent storage."
    )
